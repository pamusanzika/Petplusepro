...
// for battery voltage sensing
const int batteryPin = A0;
const float minVoltage = 0.0; // Minimum battery voltage (empty state)
const float maxVoltage = 4.2; // Maximum battery voltage (fully charged state)
const float voltageDividerRatio = 2.6;
...
{
  int sensorValue = analogRead(batteryPin);                           // Read analog input from pin A0
  float voltage = sensorValue * (7.4 / 4095.0) * voltageDividerRatio; // Convert ADC value to voltage (assuming ESP32 ADC is 12-bit)

  // Calculate percentage
  batteryPercentage = ((voltage - minVoltage) / (maxVoltage - minVoltage)) * 100;
  ...
  batteryPercentage = round(batteryPercentage / 10) * 10;
}
...
void loop2(void *pvParameters)
{
  ...
  while (1)
  {
    ...
    json.add("battery_percentage", batteryPercentage);
